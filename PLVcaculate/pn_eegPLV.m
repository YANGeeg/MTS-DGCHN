function [plv] = pn_eegPLV(eegData,  dataSelectArr)
...
plv = zeros(size(eegData, 2), numChannels, numChannels, numConditions);
for channelCount = 1:numChannels-1
    channelData = squeeze(eegData(channelCount, :, :));
    for compareChannelCount = channelCount+1:numChannels
        compareChannelData = squeeze(eegData(compareChannelCount, :, :));
